...
class Cart extends Model
{
		...
		if($countryid != '') {			
			...
			if($countrys) {
				if($countrys->taxtitle != '') {
					$taxtitle = $countrys->taxtitle.' ('.$countrys->taxpercentage.'%)';
				} else {
					$taxtitle = 'Tax ('.$countrys->taxpercentage.'%)';
				}
	...
	public function getGrandTotal($subtotal = 0, $gst = 0, $deliverycost = 0, $packingfee = 0, $discount = 0,$fuelcharges = 0, $handlingfee = 0) {		
		$grandtotal = (float)str_replace(',', '', $subtotal) + (float)str_replace(',', '', $gst) + (float)str_replace(',', '', $deliverycost) + (float)str_replace(',', '', $packingfee) + $fuelcharges + $handlingfee;
	...
	public function getPackagingFee($countrycode = '', $totalweight = '', $subtotal = 0, $gst = 0, $deliverymethod = 0, $shippingbox = '', $quantity = 1) {
		...
		if($countrycode != '') {
			...
			if($countrycode == 'SG') {
				...
				//if($freeshippingcost >= $free_shipping_amount || ($shippingmethods->shipping_type == 0 && strpos($shippingmethods->EnName, 'Ninja Van Delivery') === false)) {
					$shipamount = 0;
					$shipweightamt = 0;
					$packingfee = 0;
				} else {
	...
	public function getDiscount($subtotal = 0, $tax = 0, $shippingcost = 0, $packingfee = 0, $distype = 0, $dis = 0,$fuelcharges = 0, $handlingfee = 0) {
		$disamount = 0;
		if($distype == 1) {			
			$tmptotal = (float)str_replace(',', '', $subtotal) + (float)str_replace(',', '', $tax) + (float)str_replace(',', '', $shippingcost) + (float)str_replace(',', '', $packingfee) + $fuelcharges + $handlingfee ;
