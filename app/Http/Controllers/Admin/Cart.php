...
class Cart extends Model
{
	...
	public function getPackagingFee($countrycode = '', $totalweight = '', $subtotal = 0, $gst = 0, $deliverymethod = 0, $shippingbox = '', $quantity = 1) {
		...
		if($countrycode != '') {
			...
			if($countrycode == 'SG') {
				...
				if($freeshippingcost >= $free_shipping_amount || $shippingmethods->shipping_type == 0) {
					$shipamount = 0;
					$shipweightamt = 0;
					$packingfee = 0;
				} else {
	...
	public function shippingCost($countrycode = '', $deliverymethod = 0, $shippingbox = '', $quantity = 1, $subtotal = 0, $gst = 0) {
		...
		$countryid = $shipamt = $initial_amt = $first_comm = $second_comm = $calculated = $dhl_extra_amount = 0;
		$countrydata = Country::where('countrycode', '=', $countrycode)->select('countryid')->first();
		if($countrydata) {
			$countryid = $countrydata->countryid;
		}
		...
		if($countrycode == 'SG') {			
			...
			if($freeshippingcost >= $zones->FreeShipCost || $shippingmethods->shipping_type == 0) {
				$shipamt = '0.00';
			} else {				
				if($weight <= 5) {
