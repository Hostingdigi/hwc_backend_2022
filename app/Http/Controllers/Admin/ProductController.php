...
class ProductController extends Controller
{
    ...
    {
        $status = $Types = $Brand = $Nametype = $EnName = $Qtytype = $Qty = $productcode = $barcode = $IsPromotion = $sortcolumn = '';
        $sortby = 'Id';
        $sortorder = 'DESC';
        $join = 'Id > 0 ';
        ...
        if (isset($request->status)) {
            $status = $request->status;
            Session::put('searchStatus', $status);
        } elseif (Session::has('searchStatus')) {
            $status = Session::get('searchStatus');
        }

        if (isset($request->Types)) {
            $Types = $request->Types;
            Session::put('searchTypes', $Types);
        } elseif (Session::has('searchTypes')) {
            $Types = Session::get('searchTypes');
        }

        if (isset($request->Brand)) {
            $Brand = $request->Brand;
            Session::put('searchBrand', $Brand);
        } elseif (Session::has('searchBrand', $Brand)) {
            $Brand = Session::get('searchBrand', $Brand);
        }

        if (isset($request->Nametype)) {
            $Nametype = $request->Nametype;
            Session::put('searchNametype', $Nametype);
        } elseif (Session::has('searchNametype', $Nametype)) {
            $Nametype = Session::get('searchNametype', $Nametype);
        }

        if (isset($request->EnName)) {
            $EnName = $request->EnName;
            Session::put('searchEnName', $EnName);
        } elseif (Session::has('searchEnName', $EnName)) {
            $EnName = Session::get('searchEnName', $EnName);
        }

        if (isset($request->Qtytype)) {
            $Qtytype = $request->Qtytype;
            Session::put('searchQtytype', $Qtytype);
        } elseif (Session::has('searchQtytype', $Qtytype)) {
            $Qtytype = Session::get('searchQtytype', $Qtytype);
        }

        if (isset($request->Qty)) {
            $Qty = $request->Qty;
            Session::put('searchQty', $Qty);
        } elseif (Session::has('searchQty', $Qty)) {
            $Qty = Session::get('searchQty', $Qty);
        }

        if (isset($request->productcode)) {
            $productcode = $request->productcode;
            Session::put('searchProductCode', $productcode);
        } elseif (Session::has('searchProductCode', $productcode)) {
            $productcode = Session::get('searchProductCode', $productcode);
        }
        if (isset($request->barcode)) {
            $barcode = $request->barcode;
            Session::put('searchbarcode', $barcode);
        } elseif (Session::has('searchbarcode', $barcode)) {
            $barcode = Session::get('searchbarcode', $barcode);
        }

        if (isset($request->IsPromotion)) {
            $IsPromotion = $request->IsPromotion;
            Session::put('searchIsPromotion', $IsPromotion);
        } elseif (Session::has('searchIsPromotion', $IsPromotion)) {
            $IsPromotion = Session::get('searchIsPromotion', $IsPromotion);
        }

        if (isset($request->sortcolumn)) {
            $sortcolumn = $request->sortcolumn;
            Session::put('searchSortColumn', $sortcolumn);
        } elseif (Session::has('searchSortColumn', $sortcolumn)) {
            $sortcolumn = Session::get('searchSortColumn', $sortcolumn);
        }
        ...
        if ($barcode != '') {
            $prodoptions = ProductOptions::whereRaw("barcode LIKE '%" . $barcode . "%'")->get();
            if (isset($prodoptions)) {
                $prod_array = array();
                foreach ($prodoptions as $option) {
                    if (!in_array($option->Prod, $prod_array)) {
                        $prod_array[] = $option->Prod;
                    }
                }

            }

            $product_ids = implode(",", $prod_array);

            $join .= "AND  (barcode LIKE '%" . $barcode . "%' or Id in(" . $product_ids . "))";
        }
        ...
        $brands = Brand::where('ParentLevel', '=', 0)->orderby("EnName", 'asc')->get();
        ...
        return view('admin.Product.index', compact('products', 'categories', 'brands', 'status', 'Types', 'Brand', 'Nametype', 'EnName', 'Qty', 'Qtytype', 'productcode', 'barcode', 'IsPromotion', 'sortcolumn', 'moduleaccess', 'adminrole'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin/Product.edit', compact('product', 'brands', 'categories', 'adminrole'));
    }
    public function update(Request $request)
    {
        ...
        if ($adminrole == 5) {
            Product::where('Id', '=', $id)->update(array('barcode' => $request->barcode));
        } else {
            if ($request->hasFile('Image')) {
                $smallimage = $request->file('Image');
                $small_image = time() . '.' . $smallimage->getClientOriginalExtension();
            ...
            Product::where('Id', '=', $id)->update(array('EnName' => $request->EnName, 'UniqueKey' => $request->UniqueKey, 'ProdCode' => $request->ProdCode, 'barcode' => $request->barcode, 'Types' => $request->Types, 'Brand' => $request->Brand, 'Price' => $request->Price, 'StandardPrice' => $request->StandardPrice, 'Vendor' => $request->Vendor, 'Supplier' => $request->Supplier, 'gebiz_item' => $gebizitem, 'Color' => $request->Color, 'Size' => $request->Size, 'Specs' => $request->Specs, 'Dimension' => $request->Dimension, 'MOQ' => $request->MOQ, 'unspsc' => $request->unspsc, 'Quantity' => $request->Quantity, 'cust_qty_per_day' => $request->cust_qty_per_day, 'ShippingBox' => $request->ShippingBox, 'Weight' => $request->Weight, 'Video' => $request->Video, 'IsFeatured' => $request->IsFeatured, 'IsPromotion' => $request->IsPromotion, 'IsOverseasShippingTrue' => $request->IsOverseasShippingTrue, 'EnInfo' => $request->EnInfo, 'MetaTitle' => $request->MetaTitle, 'MetaKey' => $request->MetaKey, 'MetaDesc' => $request->MetaDesc, 'ProdStatus' => $request->ProdStatus, 'Image' => $filename, 'LargeImage' => $largefilename, 'MobileImage' => $mobilefilename, 'MobileLargeImage' => $mobilelargefilename, 'Tds' => $tdsfilename, 'Sds' => $sdsfilename, 'EnShortDesc' => $request->EnShortDesc, 'ProdTags' => $request->ProdTags));
        }
        $filterqry = '';
        if (Session::has('searchQtytype')) {
            $filterqry = "?status=" . Session::get('searchStatus') . "&Types=" . Session::get('searchTypes') . "&Brand=" . Session::get('searchBrand') . "&Nametype=" . Session::get('searchNametype') . "&EnName=" . Session::get('searchEnName') . "&Qtytype=" . Session::get('searchQtytype') . "&Qty=" . Session::get('searchQty') . "&productcode=" . Session::get('searchProductCode') . "&IsPromotion=" . Session::get('searchIsPromotion') . "&sortcolumn=" . Session::get('searchSortColumn');
        }
        return redirect('/admin/products' . $filterqry)->with('success', 'Product Successfully Updated!');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect('/admin/products' . $filterqry)->with('success', 'Product Successfully Deleted!');
    }
    ...
    public function updatestatus($id, $status)
    {
        ...
        return redirect('/admin/products' . $filterqry)->with('success', 'Product Status Successfully Updated!');
    }
    ...
    public function productoptions($id)
    {
        ...
        return view('admin/Product.options', compact('prodoptions', 'product', 'editoption', 'id', 'adminrole'));
    }
    ...
    public function editproductoptions($id, $prodid)
    {
        ...
        return view('admin/Product.options', compact('prodoptions', 'product', 'editoption', 'adminrole'));
    }
    ...
    public function updateoptions(Request $request)
    {
        ...
        if ($adminrole == 5) {
            ProductOptions::where('Id', '=', $editid)->update(array('barcode' => $request->barcode));
        } else {
            $qty = 0;
            if (isset($request->Quantity)) {
            ...
            ProductOptions::where('Id', '=', $editid)->update(array('Title' => $request->Title, 'barcode' => $request->barcode, 'Price' => $request->Price, 'Quantity' => $qty, 'cust_qty_per_day' => $cust_qty_per_day, 'ShippingBox' => $request->ShippingBox, 'Weight' => $request->Weight, 'Status' => $request->Status));
        }
    ...
    public function bulkupdate(Request $request)
    {
        ...
        if ($bulkaction == 'delete') {
            ...
            if (is_array($productids)) {
                foreach ($productids as $productid) {
                    Product::where('Id', '=', $productid)->update(array('ProdStatus' => 1));
                }
            }
        } elseif ($bulkaction == 'status_inactive') {
            if (is_array($productids)) {
                foreach ($productids as $productid) {
                    Product::where('Id', '=', $productid)->update(array('ProdStatus' => 0));
                }
            }
        } elseif ($bulkaction == 'update_price') {
            if (is_array($productids)) {
    ...
    {
        Session::forget('searchStatus');
        Session::forget('searchTypes');
        Session::forget('searchBrand');
        Session::forget('searchNametype');
        Session::forget('searchEnName');
        Session::forget('searchQtytype');
        Session::forget('searchQty');
        Session::forget('searchProductCode');
        Session::forget('searchIsPromotion');
        Session::forget('searchSortColumn');
