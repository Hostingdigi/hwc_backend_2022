...
class ProductController extends Controller
{
	...
	public function productdetails($product, Request $request) {
		if($product) {
			...
			$relatedproducts = Product::where('Types', '=', $typeid)->where('UniqueKey', '!=', $product)->orderByRaw('RAND()')->take(6)->get();
	...
	public function getproductslist(Request $request) {
		...
		if($products) {
			foreach($products as $product) {
				...
				$content .= '$'.number_format($displayprice, 2).'</li>';
				
				if($displayprice < $actualprice) {
					$content .= '<li class="list-inline-item strikeoutprice">$'.number_format($actualprice, 2).'</li>';
				}
				
				$hoolahimg = url('/images/8dc4dab.png');
				
				$content .= '</ul>';
				$content .= '<div class="text-center mt-3">or installments of $'.number_format($installmentPrice, 2).' with <img src="'.$hoolahimg.'" style="width:46px; vertical-align: inherit;"></div>';
	...
	public function search(Request $request) {
		...
		if($join) {
			$promoproducts = Product::where('EnName', 'LIKE', '%'.$searchkey.'%')->where('ProdStatus', '=', '1')->whereRaw($join)->orderBy($sortby, 'desc')->paginate($show);
			...
			$promoproducts = Product::where('EnName', 'LIKE', '%'.$searchkey.'%')->where('ProdStatus', '=', '1')->orderBy($sortby, 'desc')->paginate($show);
