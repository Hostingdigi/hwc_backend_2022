...
class PaymentController extends Controller
{
    ...
    public function commonCurl($data)
    {
        ...
        return ['status_code' => $httpCode,'output' => $output];
    }
    
    public function grabpayInquire(Request $request){
        if($request->filled('order_id'))
        {
            ...
            $response = $this->commonCurl([
                'url' => 'https://partner-api.grab.com/grabpay/partner/v2/one-time-charge/$orderId/status?currency=SGD',
                'header' => [
                    'Content-Type: application/json',
                    'Authorization: Bearer ' . $access_token,
                    'X-GID-AUX-POP: ' . $gpay->generatePopSignature($clientSec, $access_token, $dateTime),
                    'Date: ' . $dateTime,
                ],
                'payload' => [
            ...
            echo "Order ID is must";
        }
    }
    
    public function grabpayComplete(){
        ...
        echo $clientSec;
        $access_token = 'eyJhbGciOiJSUzI1NiIsImtpZCI6Il9kZWZhdWx0IiwidHlwIjoiSldUIn0..MQCqgjFGLxQy-X39ZA25bWPrN3det1rmxlRou9I9ZOdI_KYsVb5sTHi58KvnpWlWLHpPyWVb1pVCVhv05bFk-sjv7gTNBvDcFebBysWuaZhtI-KL3PjIX-HV1DUocoVu2_DE_I_xngiYcC-6fnnluhJ0-lfC8uYszIuSbfNlfQCucWHwnYEEIDSnRFyhN7U7ABz4gEF_gDm2rkM4VfLA6K8gqe2ZYEAhzuzkrlHdP7IN2DExPdOt7XFBCQZj6uT4ROtf2s1cI5taAtm_yT7bwwftaD-lMyV2CwCSsZ6ziy5ukHrai6jf1U9ciHeN8CaNcBSd_e_NpfCoF6pHDQUr7g';
        $gpay = new GrabPayFunctions;
        $dateTime = gmdate("D, d M Y H:i:s \G\M\T");
        try{
            $postRequest = array(
            ...
            $cURLConnection = curl_init('https://partner-api.grab.com/grabpay/partner/v2/charge/complete');
            curl_setopt($cURLConnection, CURLOPT_POST, 1);
    ...
    public function success(Request $request)
    {
        ...
        if($request->has('code') || $request->has('error'))
        {
            ...
            if($request->has('code')) OrderMaster::where('order_id', '=', $orderid)->update([
                ...
                try
                {
                    ...
                    $oAuthTokenResult = $this->commonCurl([
                        ...
                        'payload' => [
                            ...
                            'redirect_uri' => url('success?orderid='.$orderid),
                    ...
                    $resultObj = json_decode($oAuthTokenResult['output']);
                    ...
                    if (isset($resultObj->access_token))
                    {
                        ...
                        $completeGrabpayPayment = $this->commonCurl([
                            ...
                            'header' => [
                                ...
                                'X-GID-AUX-POP: ' . $gpay->generatePopSignature($clientSec, $resultObj->access_token),
                        ...
                        $completeResultObj = json_decode($completeGrabpayPayment['output']);
                        Paymentlog::where('order_id',$orderid)->update([
                            'received_values' => serialize($completeResultObj),
                            'status' => $completeGrabpayPayment['status_code'] == 200 ? 'success' : 'failure'
                            ]
                        );
        ...
        if ($order) {
            ...
            if ($order->order_type == 2) OrderMaster::where('order_id', '=', $orderid)->update(array('quotation_status' => '1'));
        ...
        if ($order) {
            ...
            if ($setting) {
                ...
                try{
                    ...
                    Log::error('ORDER ID : ' . $order->order_id . ' email is not works.');
                }
            }
        }
    ...
    {
        Log::info('Grabpay hook : ' . serialize($request->all()));
    }

    public function grabpay()
    {
        $paymentmethod = PaymentMethods::where('id', '5')->first();
        if (!Session::has('billinginfo') || !$paymentmethod) return redirect('/');
        ...
        if (empty($cartdata)) return redirect('/');

		$subtotal = $cartItems['subTotal'];
		$grandtotal = $subtotal + $gst + $deliverycost + $packingfee+ $fuelcharges + $handlingfee;
		$grandtotal = $this->orderServices->roundDecimal(($grandtotal - $discount));

        //Create order
        $orderCreate = $this->orderServices->createOrder();

        if ($orderCreate['status'] == false) return redirect('cancelpayment', ['orderid' => ($orderCreate['orderId'] ?? '')]);
        
        // session()->put('old_order_id', $orderCreate['orderId']);
        ...
        if (Session::has('couponcode')) {
            $couponcode = Session::get('couponcode');
            $coupondata = Couponcode::where('coupon_code', '=', $couponcode)->where('status', '=', '1')->first();
            if ($coupondata)  $couponid = $coupondata->id;
        }

        if (!empty($couponid) && !empty($discount))  CouponCodeUsage::insert(['coupon_id' => $couponid, 'customer_id' => $userid, 'order_id' => $orderid]);

        try {

            // Payment gateway details
            define('CONST_PARTNER_SECRET', $paymentmethod->api_key);
            define('CONST_CLIENT_SECRET', $paymentmethod->api_signature);
            define('CONST_CLIENT_ID', $paymentmethod->test_api_signature);
            define('CONST_PARTNER_ID', $paymentmethod->test_api_key);
            define('CONST_MERCHANT_ID', $paymentmethod->merchant_id);
            define('ENDPOINT_URL', 'https://partner-api.grab.com/');
            define('CONST_REDIRECT_URI', url('success?orderid=' . $orderid));
            ...
            if ($output === false) {
                Log::alert('ORDER ID : ' . $orderid . ' grabpay curl error => '.curl_error($cURLConnection));
                return redirect('cancelpayment', ['orderid' => $orderid]);
            } else {

                $resultObj = json_decode($output);
                $returnRequest = $resultObj->request ?? '';
                if (!empty($returnRequest)) {
                    $wp_session['request'] = $returnRequest;
                    setcookie('grabpay_request', $returnRequest, time() + 36000);
                }
                
                curl_close($cURLConnection);

                setcookie('my_order_id', $orderid, time() + 36000);
                return redirect()->away($gpay->getAuthorizeLink($returnRequest));
                // header('location:' . $gpay->getAuthorizeLink($returnRequest));
                // exit;
            }
        } catch (Exception $e) {
            return redirect('cancelpayment', ['orderid' => $orderid]);
