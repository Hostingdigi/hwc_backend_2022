...
class CommonMobileController extends Controller
{
    ...
    {
        $allcountries = Country::select(['countryid', 'countryname', 'countrycode', 'taxtitle', 'taxpercentage'])->where('country_status', '1')->orderBy('countryname', 'asc')->get();

        return response()->json(['response' => (empty($allcountries) ? 'failed' : 'success'), 'message' => 'Countries', 'countries' => (empty($allcountries) ? '' : $allcountries)]);
    }
    ...
    public function sendfeedback(Request $request)
    {
        ...
        if ($emailtemplate) {
            ...
            $data = response()->json(['response' => 'success', 'message' => 'Feedback Successfully Sent!', 'html' => $emailcontent]);
        } else {
    ...
    {
        $data = $mobileversions = [];
        $versions = MobileVersions::orderBy('id', 'desc')->get();
        if ($versions) {
            $x = 0;
            foreach ($versions as $version) {
                $mobileversions[$x]['version_name'] = $version->version_name;
                $mobileversions[$x]['version_id'] = $version->version_id;

                ++$x;
            }
            $data = response()->json(['response' => 'success', 'message' => 'Mobile Versions', 'mobileversions' => $mobileversions]);
        } else {
            $data = response()->json(['response' => 'failed', 'message' => 'No Mobile Versions Found!', 'mobileversions' => '']);
        }
    ...
    public function sendTestEmail(Request $request)
    {
        ...
        $html = '<b>Test email</b>';
        Mail::send([], [], function ($message) use ($to_name, $to_email, $html) {
            $message->to($to_email, $to_name)
                ->subject('Test Mail')
                ->from(env('MAIL_USERNAME'), 'Test Mail')
                ->setBody($html, 'text/html');
        });

        return response()->json(['status' => true, 'message' => 'Successfully email sent']);
    }

    public function listFaqs(Request $request)
    {
        $content = PageContent::where('UniqueKey', 'faq')->first();
        return response()->json([
            'response' => 'success',
            'message' => 'FAQ Page',
            'content' => $content->ChContent ?? '',
        ]);
    }

    public function listPrivacyPolicy(Request $request)
    {
        $content = PageContent::where('UniqueKey', 'privacy-policy')->first();
        return response()->json([
            'response' => 'success',
